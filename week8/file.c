...
int bstClosest(struct node *t, int val) {
    struct node *curr = t;
    int closest = t->value;
    int closestDifference = abs(val - closest);

    while (curr != NULL) {
        int currDifference = abs(val - curr->value);

        if (currDifference < closestDifference) {
            closest = curr->value;
            closestDifference = currDifference; 
        }
        ...
        if (val < curr->value) {
            ...
            // if find exact number just return val
            return val;
        }
    }
